...
scenarios('../features/profile-service.feature')

# ------------------------ Scenario 1: Visualizar o Perfil ------------------------
...
# ------------------------ Scenario 2: Editar o Perfil ------------------------

@given(parsers.parse('existe um usuário de nome "{user_name}", id "{user_id}" 2'))
def force_user_profile_for_update(context, user_name, user_id, client):
    user_id_int = int(user_id)
    response = client.patch(f"/update_profile/{user_id_int}/{user_name}/url_antiga.jpg/biografia_antiga")
    response_data = response.json()
    assert response_data["status_code"] == 200, f"Falha ao atualizar o perfil do usuário {user_id}"
    context["user_id"] = user_id_int
    context["user_name"] = user_name
...
def send_patch_profile_request(context, endpoint, client):
    response = client.patch(endpoint)
    context["response"] = response.json()

@then(parsers.parse('o status da resposta deve ser "{expected_status}" 2'))
def check_patch_profile_status(context, expected_status):
    expected_status = int(expected_status)
    actual_status = context["response"].get("status_code")
    assert actual_status == expected_status, f"Status esperado {expected_status}, mas obtido {actual_status}"

@then(parsers.parse('o JSON da resposta deve retornar uma mensagem Http "{expected_message}" 2'))
def check_patch_profile_message(context, expected_message):
